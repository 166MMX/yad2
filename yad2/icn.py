  ...
  class Map:
      ...
      def read(self):
          ...
          for _ in xrange(ngroups):
            group_indexes.append(struct.unpack('H', self.stream.read(2))[0]*2)
          ...
          for i in xrange(len(group_indexes)-1):
            ...
            self.logger.debug("group %d start %d end %d size %d" % (i+1, group_indexes[i], group_indexes[i+1], size))
            g = []
            try:
  ...
  class Index:
      ...
      class Chunk:
          ...
          def _tokenize(self, stx):
              st = array('c', stx[::-1])
              return Icn.Index.Util.reverse_bytes(ord(st[0]), ord(st[1]), ord(st[2]), ord(st[3]))
